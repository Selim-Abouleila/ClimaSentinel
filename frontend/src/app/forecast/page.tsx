    ...
    <main className="min-h-screen p-6 md:p-12 lg:p-20">
      <div className="max-w-7xl mx-auto space-y-12">
        ...
        <div className="glass-panel rounded-xl p-3 flex flex-wrap gap-2 items-center justify-center md:justify-start border border-white/5 shadow-lg">
          <span className="text-xs font-semibold uppercase tracking-wider text-slate-500 mr-2 ml-2">Region:</span>
          {ALL_CITIES.map((c) => {
        ...
        {!forecast ? (
          <div className="glass-panel rounded-xl p-12 text-center mt-12 border border-white/5">
            <h2 className="text-xl font-medium mb-2 text-slate-300">Forecast Unavailable</h2>
            <p className="text-sm text-slate-500">Unable to retrieve ML model inference for {currentCityObj.name}. Verify backend connection.</p>
          </div>
        ) : (
          <div className="space-y-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              ...
              <div className="glass-panel rounded-xl p-6 flex flex-col justify-between border border-white/5 relative overflow-hidden">
                <span className="text-xs font-semibold uppercase tracking-wider text-slate-400 mb-4">Est. Total Risk (Day +3)</span>
                <div>
                  ...
                  <div className="mt-3 inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-slate-900 border border-slate-800 text-[11px] font-medium text-slate-400">
                    95% CI: <span className="text-cyan-400 font-semibold">[{forecast.total_ci_lower} - {forecast.total_ci_upper}]</span> (±{forecast.total_confidence_margin})
                  </div>
                </div>
              </div>
              ...
              <div className="glass-panel rounded-xl p-6 flex flex-col justify-between border border-white/5">
                <span className="text-xs font-semibold uppercase tracking-wider text-slate-400 mb-4">Forecasted Driver</span>
                <div>
              ...
              <div className="glass-panel rounded-xl p-6 flex flex-col justify-between border border-white/5">
                <span className="text-xs font-semibold uppercase tracking-wider text-slate-400 mb-4">Today's Baseline (t0)</span>
                <div>
                  ...
                  <div className="text-xs font-medium text-slate-400 flex items-center gap-1">
                    {forecast.estimated_total_tipping_score > forecast.current_tipping_score ? (
                      <span className="text-amber-400">Projected increase of {(forecast.estimated_total_tipping_score - forecast.current_tipping_score).toFixed(1)}</span>
                    ) : (
                      <span className="text-emerald-400">Projected decrease of {(forecast.current_tipping_score - forecast.estimated_total_tipping_score).toFixed(1)}</span>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="glass-panel rounded-xl p-6 flex flex-col justify-between bg-slate-900/60 border border-white/5">
                <span className="text-xs font-semibold uppercase tracking-wider text-slate-400 mb-3 flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-cyan-400 font-medium bg-cyan-500/10 px-2 py-0.5 rounded border border-cyan-500/20">Open-Meteo</span>
                </span>
                <div className="space-y-2 text-xs font-normal text-slate-400">
                  <div className="flex justify-between items-center py-1 border-b border-white/5">
                    <span>Tomorrow (Day +1):</span>
                    <span className="font-medium text-slate-200">{forecast.weather_trajectory_3d.temp_max_plus_1d.toFixed(1)}°C</span>
                  </div>
                  <div className="flex justify-between items-center py-1 border-b border-white/5">
                    <span>Day +2 Max Temp:</span>
                    <span className="font-medium text-slate-200">{forecast.weather_trajectory_3d.temp_max_plus_2d.toFixed(1)}°C</span>
                  </div>
                  <div className="flex justify-between items-center py-1 border-b border-white/5">
                    <span>Day +3 Max Temp:</span>
                    <span className="font-medium text-slate-200">{forecast.weather_trajectory_3d.temp_max_plus_3d.toFixed(1)}°C</span>
                  </div>
                  <div className="flex justify-between items-center py-1">
                    <span>Day +3 Storm Risk:</span>
                    <span className="font-medium text-cyan-400">{forecast.weather_trajectory_3d.precip_plus_3d.toFixed(1)} mm</span>
                  </div>
                </div>
              </div>
            ...
            <section className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                {[
                  { title: 'Heat Score Forecast', key: 'heat_score', desc: 'Predicting severe maximum temperature anomalies and upward thermal velocity.' },
                  { title: 'Wind Score Forecast', key: 'wind_score', desc: 'Predicting severe wind gust velocity exceeding 40 km/h baseline thresholds.' },
                  { title: 'Rain Score Forecast', key: 'rain_score', desc: 'Predicting severe 24-hour accumulated precipitation totals.' },
                  { title: 'Air Quality Forecast', key: 'air_score', desc: 'Predicting European AQI spikes based on atmospheric stagnation.' },
                  { title: 'River Flood Forecast', key: 'river_score', desc: 'Predicting positive hydrological discharge velocity for monitored river basins.' },
                ].map((item) => {
                  ...
                  return (
                    <div key={item.key} className="glass-card rounded-xl p-6 flex flex-col justify-between space-y-6 border border-white/5 relative overflow-hidden">
                      <div>
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="flex items-center justify-between p-2.5 rounded-lg bg-slate-900/60 border border-slate-800/80 text-xs">
                          <span className="text-slate-400 font-normal">95% Confidence Interval</span>
