...
| **Validate frontend** | `npm ci`, `npm run lint` and `npm run build` validate the typed hybrid client and production bundle |
...
| **Train and register Heat/Rain model** | Calls the reusable MLOps workflow and returns the exact schema-v3 `ClimaSentinel_HeatRainForecaster` version |
| **Promote candidate model** | Runs per-horizon gates for the learned, realized Heat/Rain outputs and assigns `champion` to that exact version before Railway deployment and E2E |
| **Live E2E** | Confirms that every horizon returns the promoted version and renders the hybrid learned/rule component provenance correctly |
...
| Step | Description |
|---|---|
| **Model promotion gates** | Trains first, then executes `model/promote.py` against the exact returned version. Every learned Heat and Rain output for Day +1/+2/+3 must satisfy its configured R² and MAE gates before the `champion` alias moves. Forecast-rule Wind/AQ/River indicators are not presented as trained targets and are not included in model-quality metrics. |
| **Production deploy** | The repository currently leaves Railway production deployment disabled. Re-enabling it must keep deployment downstream of the promotion gate. |

> **Guard gate behavior:** If any learned Heat/Rain quality gate or artifact-contract check fails, the alias is not moved and deployment remains blocked.
...
| **Train & register model** | Trains six independent outputs (realized Heat and Rain × Day +1/+2/+3), validates schema v3, registers one atomic `ClimaSentinel_HeatRainForecaster` artifact, and returns its exact concrete version |
...
the versioned snapshot. Promotion remains the downstream model-quality gate.
...
| `test_forecast_rules.py` | Wind/AQ/River thresholds, clipping, missing-source behavior and Day +3 river use of Day +4 |
| `test_model_extraction.py` | Direct training-mart extraction, exact schema/provenance and absence of leaky filling |
| `test_ml_pipeline.py` | Schema-v3 preprocessing, six-output order, horizon slicing, four-day purge and endpoint hybrid semantics |
| `test_model_training.py` | Heat/Rain metrics, MLflow metadata, DVC provenance and registered artifact contract |
| `test_model_promotion.py` | Learned-output gates, evidence minimums, non-finite rejection and legacy-artifact rejection |
| `test_model_serving_integration.py` | Real local MLflow registry, alias/pin resolution, artifact loading, cache behavior and all three horizons |

The staging Playwright suite adds a live deployment check after the exact model
candidate has been promoted. See [End-to-End Testing](13-end-to-end-testing.md).
...
| `MLFLOW_MODEL_ALIAS` | Registered Heat/Rain model alias to resolve | `champion` |
| `MLFLOW_MODEL_VERSION` | Optional concrete version pin; takes precedence over alias | unset |
