...
WITH scored_cities AS (
    SELECT
        ...
        CASE
            WHEN current_tipping_score IS NULL THEN '⚪ Unavailable'
        ...
        END AS zone_name,
        CASE
            WHEN current_tipping_score IS NULL THEN 5
...
SELECT
    ...
    STRING_AGG(city_id, ', ' ORDER BY city_id) AS cities_in_zone,
    -- List the primary drivers causing tension in this zone
    STRING_AGG(
        DISTINCT current_primary_driver,
        ', ' ORDER BY current_primary_driver
    ) AS drivers_in_zone
FROM scored_cities
