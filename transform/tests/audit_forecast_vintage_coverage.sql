...
run_window_anomalies AS (
    SELECT
        ...
        ingestion_run_id,
        city_id,
        MIN(valid_date) AS valid_date,
        MIN(horizon_days) AS horizon_days,
        'expected_7_day_window' AS reason
    FROM {{ ref('stg_city_daily_weather_vintage') }}
    GROUP BY ingestion_run_id, city_id
    HAVING COUNT(DISTINCT valid_date) != 7
        OR MIN(horizon_days) != 0
        OR MAX(horizon_days) != 6
    ...
    FROM {{ ref('stg_city_daily_air_quality_vintage') }}
    GROUP BY ingestion_run_id, city_id
    HAVING COUNT(DISTINCT valid_date) != 5
        OR MIN(horizon_days) != 0
        OR MAX(horizon_days) != 4
