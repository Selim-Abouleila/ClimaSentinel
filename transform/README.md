...
make dbt-run          # Validate city files, seed, then run all stg + mart models
...
For an existing GCP environment, update the Cloud Shell checkout on `dev`,
populate `.env`, then run `make validate-cities`, `make dbt-run` and
`make dbt-test` from the repository root. `make dbt-run` validates city files,
seeds before running models and stops on failure. It uses the CSV already in
the checkout; it does not fetch archive data or deploy an image. Scheduled
ingestion uses the seed bundled in its image, so use `make deploy` from the updated checkout
to keep subsequent scheduled runs on the same seed version. The complete
Cloud Shell sequence is in [Doc 3](../docs/3-staging-layer.md#apply-the-cold-baseline-from-gcp-cloud-shell).
...
│   ├── city_monthly_normals.provenance.json # Cohort/column retrieval and aggregation record
...
The original 10 cities' existing baseline columns remain the historical seed
and predate a checked-in generator, so they are not claimed to be exactly
reproducible. The new cities' existing baseline columns were generated by
...
The new `normal_temperature_2m_min` column covers all 20 cities and 240
city/month rows. It averages daily `temperature_2m_min` in degrees Celsius for
each city-local calendar month over the same 2014–2023 interval, using
`models=best_match` and two-decimal rounding. Its retrieval is recorded
separately in the provenance manifest. Previously checked-in columns are
...
for verification queries and tests. The history/detail APIs now expose Cold;
visible dashboard integration and ML support remain subsequent work.
