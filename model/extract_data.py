...
def extract_data():
    ...
    # Extract weather features + current tipping score, and join with tipping score 3 days in the future
    query = f"""
        SELECT 
            f.city_id,
            f.date,
    ...
    # Drop rows where the future score or current score is null
    # Also handle missing river discharge (replace with 0)
    df['river_discharge_m3s'] = df['river_discharge_m3s'].fillna(0)
    df = df.dropna(subset=['current_tipping_score', 'future_tipping_score_3d'])
